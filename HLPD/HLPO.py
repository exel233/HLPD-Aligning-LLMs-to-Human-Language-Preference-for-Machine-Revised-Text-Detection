...
import os
import torch.nn.functional as F
from utils_HLPO import calculate_HLPO_loss
...
class ComputeScore(nn.Module):
    def __init__(self, scoring_model_name, reference_model_name, HLPOtrained=True, HLPO_beta=0.5, dataset='xsum', device='cuda', cache_dir='./models'):
        super().__init__()
        self.device = device
        ...
        def load_model(model_name, device, cache_dir, HLPOtrained=True):
            model_fullname = get_model_fullname(model_name)
        ...
        scoring_model = load_model(scoring_model_name, device, cache_dir, HLPOtrained)

        self.peft_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM, inference_mode=False, r=8, lora_alpha=32, lora_dropout=0.1
        )

        reference_model = load_model(reference_model_name, device, cache_dir, HLPOtrained)
        ...
        self.forward = self.forward_HLPO
    ...
    def get_HLPO_input(self, tokenized=None ,text=[""], labels=[""], training_module=False,temperature=1.0, top_k=50,sample=False):
        if training_module:
            ...
            crit, HLPO_input  = self.criterion_fn(logits_ref, logits_score, labels)
        else:
            with torch.no_grad(): # get reference
                ...
                logits_ref = logits_score
                crit, HLPO_input = self.criterion_fn(logits_ref, logits_score, labels)
        return crit, HLPO_input, logits_score

    def forward_HLPO(self,text):
        ...
        ref_original_crit, ref_prefered_logprob, ref_original_logits_score = self.get_HLPO_input(tokenized,original_text,labels)
        train_original_crit, train_prefered_logprob, train_original_logits_score = self.get_HLPO_input(tokenized,original_text,labels,training_module=True)
        ...
        ref_sampled_crit, ref_disprefered_logprob, ref_sampled_logits_score = self.get_HLPO_input(tokenized,sampled_text,labels,sample=True)
        train_sampled_crit, train_disprefered_logprob, train_sampled_logits_score = self.get_HLPO_input(tokenized,sampled_text,labels,training_module=True,sample=True)
        
        HLPOloss, prefered_relative_logprob, disprefered_relative_logprob, reward_accuracies, reward_margins = calculate_HLPO_loss(train_prefered_logprob,train_disprefered_logprob,ref_prefered_logprob,ref_disprefered_logprob,beta=self.beta)
        output = dict(crit=[ref_original_crit, train_original_crit, ref_sampled_crit, train_sampled_crit], loss=HLPOloss)
        return output
